...
pub fn take_until_or_all<T, Input, Error: ParseError<Input>>(tag: T) -> impl Fn(Input) -> IResult<Input, Input, Error>
where
...
pub fn take_until_or_nonempty_all<T, Input, Error: ParseError<Input>>(tag: T) -> impl Fn(Input) -> IResult<Input, Input, Error>
where
    Input: InputTake + InputLength + FindSubstring<T>,
    T: InputLength + Clone
{
...
fn split_by<T, Input, Error: ParseError<Input> >(separator: T) -> impl FnMut(Input) -> IResult<Input, Vec<Input>, Error>
where 
    Input: InputTake + InputLength + FindSubstring<T> + Clone + InputIter + Compare<T>,
    T: InputLength + Clone
{
    separated_list0(tag(separator.clone()), take_until_or_nonempty_all(separator))
}
fn map_parser_vec<I, O1, O2, E, F, G>(mut parser: F, applied_parser: G) -> impl FnMut(I) -> IResult<I, Vec<O2>, E>
where
    E: ParseError<I>,
    E: ParseError<O1>,
    F: Parser<I, Vec<O1>, E>,
...
fn map_consuming_parser<I, O1, O2, E, F, G>(parser: F, applied_parser: G) -> impl FnMut(I) -> IResult<I, O2, E>
where 
    E: ParseError<I>,
    E: ParseError<O1>,
    O1: InputLength,
...
fn take_until_through<'a, Error: 'a + ParseError<&'a str> >(separator: &'a str) -> impl 'a + FnMut(&'a str) -> IResult<&'a str, &'a str, Error> {
    terminated(take_until(separator), tag(separator))
}
fn take_until_through_or_all<'a, Error: 'a + ParseError<&'a str> >(separator: &'a str) -> impl 'a + FnMut(&'a str) -> IResult<&'a str, &'a str, Error> {
    terminated(take_until_or_all(separator), tag(separator).or(success("")))
}
fn enclosed_by<'a, Error: 'a + ParseError<&'a str> >(open: &'a str, close: &'a str) -> impl 'a + FnMut(&'a str) -> IResult<&'a str, &'a str, Error> {
...
fn parse_grbl_state<'a, Error: 'a + ParseError<&'a str> >(input: &'a str) -> IResult<&'a str, GrblState, Error>
where
    Error: FromExternalError<&'a str, ParseIntError>
{
    all_consuming(
...
fn parse_float_array<'a, Error: 'a + ParseError<&'a str> >(input: &'a str) -> IResult<&'a str, Array1<f64>, Error>
where
    Error: FromExternalError<&'a str, ParseFloatError>
{
    all_consuming(
        split_by_then(",", parse_f64)
        .map(|floats| floats.into_iter().collect())
    ).parse(input)
}
fn parse_grbl_status_part<'a, Error: 'a + ParseError<&'a str> >(input: &'a str) -> IResult<&'a str, Vec<GrblStatusPart>, Error>
where
    Error: FromExternalError<&'a str, ParseFloatError>,
    Error: FromExternalError<&'a str, ParseIntError>
{
    flat_map(
        ...
        |head| -> Box<dyn Parser<&'a str, Vec<GrblStatusPart>, Error>> {
            match head {
                "WCO" => Box::new(parse_float_array.map(|wco| vec![GrblStatusPart::WorkCoordinateOffset(wco)])),
                "Bf" => Box::new(
                    separated_pair(parse_u64, tag(","), parse_u64)
                    .map(|(planner, rx_bytes)| vec![GrblStatusPart::Planner(planner), GrblStatusPart::RxBytes(rx_bytes)])
                ),
                ...
                "FS" => Box::new(
                    separated_pair(parse_f64, tag(","), parse_f64)
                    .map(|(planner, rx_bytes)| vec![GrblStatusPart::CurrentFeed(planner), GrblStatusPart::CurrentSpindle(rx_bytes)])
                ),
                "Pn" => Box::new(all.map(|pins: &str| vec![GrblStatusPart::Pins(pins.to_string())])),
                "Ov" => Box::new(
                    tuple((parse_f64, tag(","), parse_f64, tag(","), parse_f64))
                    .map(|(feed, _, rapids, _, spindle)| vec![
                        GrblStatusPart::FeedOverride(feed * 0.01),
                        GrblStatusPart::RapidOverride(rapids * 0.01),
                        GrblStatusPart::SpindleOverride(spindle * 0.01)
                    ])
                ),
                "A" => Box::new(all.map(|accessories: &str| vec![GrblStatusPart::AccessoryState(accessories.to_string())])),
                _ => Box::new(fail)
            }
        }
    )
...
fn parse_grbl_position<'a, Error: 'a + ParseError<&'a str> >(input: &'a str) -> IResult<&'a str, GrblPosition, Error>
where
    Error: FromExternalError<&'a str, ParseFloatError>
{
    all_consuming(
...
fn parse_grbl_status<'a, Error: 'a + ParseError<&'a str> >(input: &'a str) -> IResult<&'a str, GrblStatus, Error>
where
    Error: FromExternalError<&'a str, ParseFloatError>,
    Error: FromExternalError<&'a str, ParseIntError>
{
    ...
    .and_then(
        take_until_through("|").and_then(parse_grbl_state)
        .and(
            take_until_through_or_all("|").and_then(parse_grbl_position)
            .and(
                split_by_then("|", parse_grbl_status_part)
            )
        )
    ).parse(input);
...
fn parse_grbl_square_brackets<'a, Error: 'a + ParseError<&'a str> >(input: &'a str) -> IResult<&'a str, GrblMessage, Error>
where
    Error: FromExternalError<&'a str, ParseFloatError>,
    Error: FromExternalError<&'a str, ParseIntError>
{
    ...
    .and_then(
        flat_map(
            ...
            |head| -> Box<dyn Parser<&'a str, GrblMessage, Error>> {
                match head {
                    "PRB" => Box::new(
                        separated_pair(parse_float_array, tag(":"), parse_u64.map(|u| u != 0))
                        .map(|(position, success)| GrblMessage::ProbeEvent { success, position })
                    ),
                    _ => Box::new(fail)
                }
            }
...
fn parse_grbl_ok<'a, Error: 'a + ParseError<&'a str> >(input: &'a str) -> IResult<&'a str, GrblMessage, Error> {
    tag("ok").map(|_| GrblMessage::GrblOk).parse(input)
}
fn parse_grbl_error<'a, Error: 'a + ParseError<&'a str> >(input: &'a str) -> IResult<&'a str, GrblMessage, Error>
where
    Error: FromExternalError<&'a str, ParseIntError>
{
    preceded(tag("error:"), parse_u64).map(GrblMessage::GrblError).parse(input)
}
fn parse_grbl_alarm<'a, Error: 'a + ParseError<&'a str> >(input: &'a str) -> IResult<&'a str, GrblMessage, Error>
where
    Error: FromExternalError<&'a str, ParseIntError>
{
    preceded(tag("ALARM:"), parse_u64).map(GrblMessage::GrblError).parse(input)
}
fn parse_grbl_greeting<'a, Error: 'a + ParseError<&'a str> >(input: &'a str) -> IResult<&'a str, GrblMessage, Error> {
    tag("Grbl").and(all).map(|_| GrblMessage::GrblGreeting).parse(input)
}
fn parse_grbl_line_impl<'a, Error: 'a + ParseError<&'a str> >(message: &'a str) -> IResult<&'a str, GrblMessage, Error>
where
    Error: FromExternalError<&'a str, ParseFloatError>,
    Error: FromExternalError<&'a str, ParseIntError>
{
...
pub fn parse_grbl_line(message: &str) -> GrblMessage {
    parse_grbl_line_impl::<()>(message).expect("parsing always succeeds!").1
}
...
mod tests {
    ...
    */
    ...
    fn test_map_enclosed_by_split_by() {
        let input = "<a|b|c>";
        let result: Result<_, nom::Err<()>> = enclosed_by("<", ">").and_then(split_by("|")).parse(input);
    ...
    fn test_map_enclosed_by_take_first_split_by() {
        ...
        let result: Result<_, nom::Err<()>> = (
            enclosed_by("<", ">")
            .and_then(
                take_until_through("|").and(split_by("|"))
            )
        ).parse(input);
    ...
    fn test_parse_grbl_status() {
        ...
        let mut status = GrblStatus::new(GrblState::Idle, GrblPosition::Machine(array![0.0, 1.0, 3.0]));
    ...
    fn test_parse_grbl_status_3() {
        ...
        let result: Result<_, nom::Err<VerboseError<_>>> = parse_grbl_status(input);
        let status = GrblStatus::new(GrblState::Idle, GrblPosition::Machine(array![0.0, 1.0, 3.0]));
        assert_eq!(result, Ok(("", status)));
    }
    #[test]
